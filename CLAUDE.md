...
- Utilize destructuring assignment whenever possible
- Use snake_case for function names and PascalCase for class names
- You must strictly follow rules above

## Workflow

- Always run type checking after completing changes
- Prioritize unit tests over running the full test suite

## Development Style - Specification-Driven Development with Test-Driven Development (TDD)

### Test-Driven Development (TDD)

- As a general rule, proceed with Test-Driven Development (TDD)
- Create tests first based on the expected input and output
- Do not write implementation code yet; only prepare the tests
- Run the tests and confirm that they fail
- Once it's confirmed that the tests are correct, commit them
- Then proceed with the implementation to make the tests pass
- While implementing, do not change the tests—continue modifying the code
